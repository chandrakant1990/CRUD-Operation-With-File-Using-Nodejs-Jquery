...
var storage = multer.diskStorage({
  ...
  filename: function(req, file, cb) {
    ...
    var random_name =
      "" + current_year + current_month + day + hours + mins + secs;
...
app.post("/addUSerDetails", middleware, upload.single("image"), function(
  req,
  res
) {
  ...
  if (sno != null && sno != "") {
    ...
    if (userDetails.length > 0) {
      uid = d3.max(userDetails,d=>parseInt(d.sno));
      uid=uid+1;
    }
    const data = {
      sno: uid,
...
app.post("/removeUserDetails", middleware, function(req, res) {
  ...
  userIDs = userIDs.map(d => parseInt(d));

  const removedUserImages = userDetails
    .filter(d => {
      return userIDs.indexOf(d.sno) != -1;
    })
    .map(d => d.filename);
  removedUserImages.forEach(async (d, index) => {
    if (fs.existsSync(directory + d)) {
      fs.unlinkSync(directory + d);
    }
  });
...
app.get("/removeAllFiles", middleware, function(req, res) {
  removeALlUSerPhotos();
  res.send("Success");
});
